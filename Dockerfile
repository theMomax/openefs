...
RUN apt-get update && apt-get install apt-utils -y && apt-get install wget -y



# ========= install golang =====================================================
# see https://github.com/docker-library/golang/blob/f30f0428221b94c7dcb414ebdcea83106df20897/1.13/alpine3.10/Dockerfile
...
RUN  wget -c https://storage.googleapis.com/tensorflow/libtensorflow/libtensorflow-cpu-linux-x86_64-1.15.0.tar.gz -O - | tar -xz -C /usr/local && \
    ldconfig
...
COPY python/mnist.py python/
RUN cd python && python mnist.py
...
# WORKDIR /root/
# COPY --from=builder /usr/src/openefs/openefs .
# CMD ["./openefs"]  